...
(function (global, factory) {
   ...
   * split tree by point
   *
   * @param {Node} netplususer - split netplususer
   * @param {BoundaryPoint} point
   * @param {Object} [options]
   * @param {Boolean} [options.isSkipPaddingBlankHTML] - default: false
   * @param {Boolean} [options.isNotSplitEdgePoint] - default: false
   * @return {Node} right node of boundaryPoint
   */
  function splitTree(netplususer, point, options) {
      // ex) [#text, <span>, <p>]
      var ancestors = listAncestor(point.node, func.eq(netplususer));
      if (!ancestors.length) {
          return null;
      }
      ...
      Bullet.prototype.releaseList = function (clustereds, isEscapseToBody) {
          ...
          $$1.each(clustereds, function (idx, paras) {
              ...
              if (headList.parentNode.nodeName === 'LI') {
                  ...
                  // remove empty lists
                  var netplususerLists = lists.compact([headList, middleList, lastList]);
                  $$1.each(netplususerLists, function (idx, netplususerList) {
                      var listNodes = [netplususerList].concat(dom.listDescendant(netplususerList, dom.isList));
